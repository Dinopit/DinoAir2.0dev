...
# -*- coding: utf-8 -*-
"""
Start/stop watchdog lifecycle check.

- Starts the Qt-compatible watchdog via the compatibility adapter
- Waits briefly to receive metrics
- Stops the watchdog and repeats once (restart path)
- Verifies there are no lingering non-daemon threads (besides MainThread)

Exit codes:
  0 -> Clean shutdown (no lingering non-daemon threads)
  1 -> Lingering non-daemon threads detected
"""
...
# Ensure project root is on sys.path so `src` imports resolve when executed directly
...
def metrics_cb(metrics: SystemMetrics) -> None:
    # Keep it concise; just confirm flow
    print(
...
def main() -> int:
    ...
    # Baseline threads (expect only MainThread)
    ...
    # Cycle 1: start -> wait -> stop
    print("-- Cycle 1: start -> wait -> stop")
    adapter.start_monitoring()
    time.sleep(2.5)  # let a couple loops run
    ...
    # Final settle time to let threads wind down
    time.sleep(0.75)
